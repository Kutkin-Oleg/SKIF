...
if __name__ == '__main__':
    ...
    focus_formula=[]
    for temp_crl_y_t in crl_y_t_map:
        # en=tempE
        crl_y_t=temp_crl_y_t
        focal_dist_calc = crl_y_g * crl_y_t / (crl_L * np.real(1. - crl_mat.get_refractive_index(en)))
        focal_dist = focal_dist_calc
        data_dir = os.path.join(data_dir0, f'зависимость от crl_y_t-{temp_crl_y_t}')
        plots = [
    ...
    fig = mpl.pyplot.figure()
    ax = fig.add_subplot()
    ax.set_xlabel("crl_y_t , мм")
    ax.set_ylabel("Фокусное расстояние, мм")
    ax.plot(crl_y_t_map, focus_dict, 'x', label='xrt')
    ax.plot(crl_y_t_map, focus_formula, label='analitic')
    ax.legend()
    fig.savefig(os.path.join(data_dir, '..', 'focus(crl_y_t ).png'))